...
export function acquireInstance(dataDir: string): () => void {
  ...
  if (process.platform === 'win32') {
    ...
    if (heldHere.has(scope)) { library.close(); throw new Error('A Meshrooms daemon already owns this data directory.'); }
    ...
    if (heldHere.has(scope)) throw new Error('A Meshrooms daemon already owns this data directory.');
    ...
    if (library.symbols.flock(fd, 2 | 4) !== 0) {
      closeSync(fd); library.close(); throw new Error('A Meshrooms daemon already owns this data directory.');
    }
